...
namespace ImGuiX::Widgets {
    ...
    struct IconButtonConfig {
        ImVec2  size{0, 0};           // (0,0) => квадрат с высотой = GetFrameHeight()
        float   rounding{-1.0f};      ///< скругление фона
        ImFont* font{nullptr};        // шрифт (например, иконок); null => текущий
        ImVec2  text_offset{0, 0};    // тонкая подстройка базлайна/смещение текста
        bool    draw_border{false};   // рисовать рамку
        ImU32   border_col{0};        // 0 => ImGuiCol_Border
        float   border_thickness{1.0f};
    };

    /// Кнопка с произвольной строкой (в т.ч. иконкой), выровненной по центру.
    /// Возвращает true при клике ЛКМ.
    inline bool IconButtonCentered(const char* id,
		...
		if (sz.x <= 0.0f || sz.y <= 0.0f) {
			...
			sz = ImVec2(h, h);                 // квадрат по высоте текущего фрейма
		}
