...
- Если аргумент использует тернарный оператор `?:`, переносите после `?` и перед `:`; оба результата сдвигайте ещё на +4 пробела относительно строки аргумента (итого 12 от строки вызова), как в примере ниже.
...
### Doxygen
- Используйте комментарии `///` с тегами через обратный слэш (`\brief`, `\param`, `\return`, `\note`).
